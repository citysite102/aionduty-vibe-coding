...
| 判準 | 學員學完會什麼 | 這一段涵蓋哪些主題 |
| 句型 | 陳述、對比、情境、動作、提問（見下方） | 名詞短語，必要時「主題：子題」 |
| 例 | 「Git 幫你記下每一版，不只是程式碼」 | 「Git：版本控制」 |
| 例 | 「照你的目標，挑一種 AI 工具」 | 「AI 開發工具的六種類型與選用時機」 |

理由是讀者的處境不同。投影片標題是學員一頁一頁讀過去的，他在**讀**，所以要站在他學完會什麼的位置說話。章節名與單元名出現在講者的下拉選單、`npm run units` 印出來的錄製清單、影片檔名與課程平台的目錄，他在那裡是**在找東西**：想知道「Git 在哪一支」「權限模式在哪一支」，能力導向的長句會讓關鍵字掃不到。
...
**標題的主詞應該是學員的能力，不是這一頁的內容。** 這是寫標題的第一個決定，比句型重要。

同一頁可以有兩種寫法，差別不在字漂不漂亮，在站在誰的位置說話：

| 切角 | 例 | 問題 |
|---|---|---|
| 這一頁有什麼（摘要） | 「這七樣，各解掉一個會卡住你的問題」 | 讀者知道這頁列了七項，但不知道自己學完會多出什麼能力 |
| 學員要會什麼（教學目標） | 「Vibe Coding 需要哪些基礎？先看這七個」 | 一眼看得出這一段在補哪一塊能力 |

判準：**把標題唸完，接得上「所以我學完會⋯」嗎？**接不上就是還站在摘要的位置。

這條的實務效果是，標題會自然帶出這一頁在整門課裡的位置（這是哪一類的東西、補的是哪一塊），而不只是把內容濃縮一次。

---

切角定了之後，再看形式。標題有兩條硬性條件，符合其中一條就行：

1. **有一個主要動詞，而且你指得出那個動詞的主詞是誰。**
2. **是「A，B」的並列對照**：兩半詞性相同、地位對等，各自都是看得見的東西或動作，而且拿掉任何一半，另一半就沒有對照對象。

例外有四種頁：封面、**分節頁**、案例首頁（`案例一：器 VESSEL`）、結語。這四種的職務是開一個名字，不是傳達一個命題。
...
第二條很容易被拿來當免死金牌，所以它自己帶一個判準：**「對象＋它的性質」不算並列。** 判斷方法是把兩半對調，讀起來還通嗎？

- 「兩把金鑰，兩層權限」→ 對調成「兩層權限，兩把金鑰」，通。合格。
- 「同一批資料，兩種擺法」→ 對調成「兩種擺法，同一批資料」，不通，因為後半是在說前半，不是跟它對照。不合格。

2026-08-21 全片掃過一次，只有一頁靠第二條過關（`29i_Case3_Trust` 的「兩把金鑰，兩層權限」），其餘 33 個名詞短語沒有一個通得過對調測試。**這個豁免只開給真正的並列，不要用它放行「某某的三個重點」這種標題。**

#### 決定用哪一種句型，看那一頁的職務

| 這一頁在做什麼 | 句型 | 動詞的主詞是誰 | 例 |
|---|---|---|---|
| 教一個結論 | 陳述句 | 這一頁在講的東西 | 「規則沒生效的三種原因」 |
| 有兩邊要對照 | 對比句 | 兩半各自 | 「Hook 是程式在擋，不是它記得」 |
| 引出一個學員會撞到的狀況 | 情境開場 | 學員的處境 | 「手機打開，紀錄卻是空的」 |
| 叫學員動手 | 動作句 | 學員（省略的「你」） | 「動手搭建運作框架」 |
| 承上啟下的一問 | 提問句 | ― | 一節最多兩頁 |

驗收方式：**把標題唸一次，指出主要動詞，說出主詞是誰。**指不出動詞，而這一頁又不是上面那四種例外，就要改。

**提問句與情境開場不受第一條限制。**「Vibe Coding 需要哪些基礎？」沒有主要動詞，但它是合法的，因為它的判準是另一個：那個問題或那個狀況，學員真的會有嗎？問一個學員不會問的問題，比沒有動詞更糟。

#### 一章裡不要有三個以上標題用同一個句型骨架

這條是用來擋前面那兩條的副作用的。2026-08-22 為了讓標題「都有動詞」，一口氣改出五個長這樣的：
...
2026-08-21 把全片 170 個標題分類過一次：動作句 54 頁、陳述句 52 頁，兩者合計 63%，那是這份簡報真正的主體，本身沒有問題。問題是**名詞短語有 34 頁（20%）**，扣掉分節頁、案例首頁與結語，還有 27 頁散在內容頁裡，跟前後隨機交錯。

讀者感覺到的不是「有名詞短語這一類」，是**規律突然斷掉**：連著兩頁在講結論或叫你動手，第三頁忽然只丟幾個名詞。當時列得出 16 個這種接縫。

最傷的是**系列首頁**。「手冊健檢」「規則怎麼寫」「三個角色」這三個系列，底下每一頁都是陳述句或動作句，只有第一頁是名詞短語，等於一整個系列的第一句話是最沒資訊的那一句。

反過來說規律是做得到的：`21e` 到 `21f2` 連續 11 頁全是陳述句、`harness/11` 到 `16b` 連續 8 頁全是動作句、`25` 到 `26e` 連續 9 頁全是動作句，那幾段讀起來明顯比其他地方順。
