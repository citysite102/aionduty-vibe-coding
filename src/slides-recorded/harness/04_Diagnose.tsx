...
export const meta: RecordedMeta = {
  ...
  title: '那要怎麼知道是哪一種？',
  script:
    '這三種原因的處理方式完全不同，所以不要一發現它沒照做就急著再加一條規則，那只會讓檔案更肥，原本的問題還在。診斷順序是這樣：第一步跑 /context，確認那份手冊在不在載入清單裡，不在就是位置的問題。第二步問它依據哪一條做決定。如果它答不出來，代表規則寫得無法判定。如果它答得出來卻還是做錯，那多半是被埋在後面，或是兩條規則互相矛盾。這四個步驟裡只有第三步指向句子寫壞，其餘都指向位置問題，所以下一步先決定位置。',
  seconds: 47,
  from: 68,
};
        ...
        */}
        <AnimatedBlock stepIndex={5} className="bg-amber-500/5 border border-amber-500/20 rounded-2xl px-7 py-5">
          ...
          <p className="text-slate-300 text-lg leading-relaxed mt-3 pt-3 border-t border-amber-500/15">
            四個步驟裡只有第 3 步指向句子寫壞，<Key>其餘都指向位置問題</Key>，所以下一步先決定位置。
          </p>
        </AnimatedBlock>
      </RecPage>
    </SlideLayout>
