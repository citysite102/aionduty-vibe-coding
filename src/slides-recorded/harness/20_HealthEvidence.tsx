 ...
 * 所以改成三個帶方框的格子，每格是一個要回答的問題，那條門檻寫成格子上面那一行大字。
 *
 * 方框不做成可點的：預錄頁不放互動，學員看的是影片，點不到。
...
export const meta: RecordedMeta = {
  ...
  script:
    '憑印象刪會刪錯，所以要有證據，三種。第一種現場證據：最近的對話裡，它真的違反過這條嗎？從來沒違反過的規則，可能是在防一個不存在的問題。第二種機制證據：這件事是不是已經有 Hook 或 lint 在擋？有的話文字版就是重複的。第三種時效證據：規則綁的那個資料夾、流程還在嗎？專案改過名字，規則常常沒跟著改。三格裡至少要湊得出兩格，才動手刪。只有一格就刪，你多半會刪到還在用的。',
  seconds: 42,
  from: 69,
};
