...
/**
 * 2026-09-22 換開場。這一頁是單元 5-5 的第一頁，也就是那支影片的第一個畫面，
 * 原本開頭是「光講沒有體感，所以把前面那次驗收攤開來」，接的是上一支影片的結尾，
 * 單獨播放時讀者不知道在攤開什麼。改成自己說明白：還是那個計時器專案，同一句指令丟兩次。
 *
...
export const meta: RecordedMeta = {
  ...
  title: '同一句話，有沒有手冊差在哪',
  script:
    '還是你那個計時器專案，同一句指令丟兩次，差別只有有沒有一份 CLAUDE.md。指令是「幫我加一個 5 分鐘的休息模式」，兩次都沒提任何規矩。左邊是手冊還沒寫的時候：按鈕寫成開始休息，不是那套航太語彙；塞了一張外部圖片當背景；分鐘數又寫死在程式碼裡，三條全沒中。右邊只多了一份 CLAUDE.md：按鈕自己叫補給，沒有引用外部圖片，分鐘數也加在最上面的設定區。三條全中。',
  seconds: 38,
  from: 55,
};
      ...
      <RecPage className="space-y-6">
        ...
        <AnimatedBlock stepIndex={2} className="grid grid-cols-2 gap-5 items-stretch">
          <div className="rounded-2xl border border-red-500/20 bg-red-950/20 p-6 flex flex-col">
            <div className="text-slate-500 text-sm mb-1">專案裡沒有 CLAUDE.md</div>
            <div className="text-red-300 text-lg font-bold mb-5">三條一條都沒中</div>
            <ul className="space-y-3 mt-auto">
              {MISSES.map((m) => (
                <li key={m} className="text-slate-400 text-base leading-snug flex gap-3">
          ...
          <div className="rounded-2xl border border-emerald-500/25 bg-emerald-950/20 p-6 flex flex-col">
            <div className="text-slate-500 text-sm mb-1">同一個專案，多了 CLAUDE.md</div>
            <div className="text-emerald-300 text-lg font-bold mb-5">這一次三條全中</div>
            <ul className="space-y-3 mt-auto">
              {HITS.map((h) => (
                <li key={h} className="text-slate-200 text-base leading-snug flex gap-3">
