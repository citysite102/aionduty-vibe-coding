...
export const meta: RecordedMeta = {
  ...
  script:
    '它長什麼樣？你寫的是一段設定，不用自己寫程式，而且拆開只有三層。第一層問什麼時候檢查，叫 Event。第二層問這一次要不要管，叫 Matcher。第三層問到底做什麼，叫 Handler。用這份簡報那一條對回去：時機是它要寫檔案之前，範圍是只有寫檔案跟改檔案要管，動作是擋下來並告訴它哪裡不行。下面那塊是它在檔案裡的樣子，三個名詞對三個位置。三層要寫齊。沒寫範圍它照樣能跑，但每一次用工具都會跳出來檢查，你很快就會想把它關掉。',
  seconds: 45,
};
