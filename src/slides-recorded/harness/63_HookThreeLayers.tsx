...
/**
 * **口白刻意超過 45 秒（54 秒），不要砍回去。** 理由同 62_HookHowTo：
...
export const meta: RecordedMeta = {
  ...
  script:
    '你寫的是一段設定，不用自己寫程式，而且拆開只有三層。第一層問什麼時候檢查，叫 Event。第二層問這一次要不要管，叫 Matcher。第三層問到底做什麼，叫 Handler。三個英文名詞聽起來很多，但拿這份簡報掛的那一條套進去，你就知道它在講什麼了。什麼時候檢查？它要寫檔案進去之前。這一次要不要管？只有寫檔案跟改檔案要管，它讀東西的時候不要來煩我。做什麼？把這次擋下來，然後告訴它哪裡不行。畫面下面那段設定檔，這三句話就對應到三個位置，你打開自己那一份也會看到同樣的三行。三層要寫齊，最常漏掉的是中間那層範圍。',
  seconds: 54,
};
