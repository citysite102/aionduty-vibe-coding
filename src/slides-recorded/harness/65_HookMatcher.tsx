...
export const meta: RecordedMeta = {
  ...
  title: '第二層：只留你要管的那一次',
  script:
    '第二層是範圍。時機到了，這一輪它可能在讀檔案、跑指令，也可能在寫檔案，範圍那一行決定哪幾種要管。畫面上那一行就是實際的寫法，下面那排是結果：寫檔案跟改檔案會被檢查，讀東西不會被打擾。這一層不寫也能跑，所以最容易被跳過，代價是它每一次都插手，然後你會想把它關掉。範圍也不是越大越好。這條原本沒有判斷檔案路徑，連專案自己的工具腳本都被擋了三次，後來在檢查的那段指令裡加上只管投影片那一區才解決。範圍這一層管的是哪個工具，路徑要在指令裡自己判斷。',
  seconds: 45,
};
