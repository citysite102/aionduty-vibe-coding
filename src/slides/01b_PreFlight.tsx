import { PackageCheck, MonitorDown, CreditCard, GitBranch, SquareTerminal, Github } from 'lucide-react';
      ...
      <div className="max-w-5xl mx-auto space-y-4 pb-6">
        ...
        >
          {REQUIRED.map((r) => {
            const Icon = r.icon;
            return (
              <div key={r.name} className="flex items-start gap-4 px-6 py-4">
        ...
        <AnimatedBlock
          stepIndex={3}
          className="rounded-2xl border border-slate-800 bg-slate-950 px-6 py-4 flex items-start gap-4"
        >
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-baseline gap-x-3">
              <span className="text-base font-bold text-slate-300">Warp</span>
              <a
            ...
            <p className="text-slate-500 text-sm leading-relaxed mt-1">
              比系統內建好用的終端機。終端機那一段會用到，不裝也走得完，用內建的就可以。
            </p>
          </div>
        </AnimatedBlock>
