 ...
 * 這一頁的職務是「這一段要解掉什麼」，那一塊卻在解釋一個名詞，而且是這一章
 * 後面 5-2 整個單元的主題，放在這裡等於先講一次。同一個名詞講兩遍，
 * 學員第二次聽到只會覺得跳過。要補回來的話請放 5-2，不要放這裡。
 *
 * 第四列原本是「不知道一個月要多少錢／算得出量級也知道怎麼壓」。這一頁的標題是
 * 「每次開新對話，你都要重講一次規矩」，前三列都在講規矩，第四列跳到錢，
 * 讀起來像硬塞進來的。錢確實是這一章的內容（5-2 講計費），但它要接得上這一頁的框。
 * 改成從規矩接到載入：規矩越多、每次帶進去的東西越多，那正是成本的來源，
 * 也是 5-7「載入時機與存放位置」在處理的事。
 *
