 ...
 * 收尾那頁（28b 新專案的前三個動作）講的是怎麼開工，沒有講做什麼，所以那一票沒有被接住。
 *
 * 位置緊接在出貨那一串之後、「新專案的前三個動作」之前，兩頁合成同一個錄製單元。
 * 它曾經被往前搬到教學段落結束的地方，理由是現場最後一段的時間永遠被前面吃光，
 * 那一票就接不到。改成一單元一支影片之後那個理由消失了，而下一頁第 1 步
 * 開頭就是「題目挑好之後」，那句話指的就是這一頁，所以搬回它旁邊。
 * 要再動位置之前，先確認那句承接語還指得到（CLAUDE.md B-4）。
 *
